...
Die Skripte liegen in [`.husky/`](https://github.com/it-at-m/agile-test-automation-framework/tree/main/.husky) (`pre-commit`, `commit-msg`).
...
Die Ticket-Nummer ist **optional** — `ATAF-123` oder nur `ATAF` (Großbuchstaben).

**Gültige Types:** `feat`, `fix`, `clean`, `chore`, `docs`

**Gültige Projekte:** `ATAF`, `GH`
...
```bash
git commit --no-verify -m "chore(ATAF): emergency hotfix"
```
...
- [Mitwirken und Lizenz](./contributing.md)
