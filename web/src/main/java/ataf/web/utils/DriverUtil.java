...
public final class DriverUtil {
    ...
    public static RemoteWebDriver getDriver() {
        RemoteWebDriver driver = REMOTE_WEB_DRIVER_MAP.get(Thread.currentThread().threadId());
        if (driver == null) {
            ScenarioLogManager.getLogger()
                    .error("No WebDriver instance for thread [{}]. Possible missing 'web' tag in Cucumber scenario.", Thread.currentThread().threadId());
            throw new WebDriverNotInitializedException("For thread [" + Thread.currentThread().threadId()
                    + "] no WebDriver instance was created! Did you forget to add the 'web' tag to your Cucumber scenario?");
        }
        return driver;
    }
    ...
    public static RemoteWebDriver initDriver(boolean isLocalExecution) throws MalformedURLException {
        ...
        if (isLocalExecution) {
            ...
            IS_LOCAL_EXECUTION_MAP.put(Thread.currentThread().threadId(), true);
        } else {
            ScenarioLogManager.getLogger().info("Test will be executed on Selenium Grid!");
            IS_LOCAL_EXECUTION_MAP.put(Thread.currentThread().threadId(), false);
        }
        ...
        switch (BROWSER) {
            case "firefox":
                ...
                if (Files.exists(EXTENSION_DIRECTORY)) {
                    try {
                        Files.walkFileTree(EXTENSION_DIRECTORY, new SimpleFileVisitor<>() {
                            ...
                            @Override
                            public @NotNull
                            FileVisitResult visitFileFailed(@NotNull Path file, @NotNull IOException exc) {
                ...
                if (isLocalExecution) {
                    ...
                    try {
                        driver = new RemoteWebDriver(new URI(SELENIUM_GRID_URL).toURL(), firefoxOptions);
            ...
            case "chrome":
                ...
                if (isLocalExecution) {
                    ...
                    try {
                        driver = new RemoteWebDriver(new URI(SELENIUM_GRID_URL).toURL(), chromeOptions);
                    } catch (URISyntaxException e) {
                        ScenarioLogManager.getLogger().error(
                                "Given value of {} could not be parsed as a URI reference. Check your property if it is a correct URL!", SELENIUM_GRID_URL);
                        CustomAssertions.fail(e.getMessage(), e);
    ...
    public static boolean isLocalExecution() {
        return IS_LOCAL_EXECUTION_MAP.get(Thread.currentThread().threadId());
    }
    ...
    public static void closeDriver() {
        ...
        REMOTE_WEB_DRIVER_MAP.remove(Thread.currentThread().threadId());
    }
}
