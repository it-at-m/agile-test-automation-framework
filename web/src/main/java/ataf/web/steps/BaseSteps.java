...
public class BaseSteps {
    ...
    /**
     * Generic Cucumber step to click on a button specified by a locator.
     *
     * @param locatorType the locator strategy to use (must match a value of {@link LocatorType}, e.g.
     *            {@code ID}, {@code XPATH})
     * @param value the locator value (e.g. an id, XPath, CSS selector, link text) used to identify the
     *            button
     */
    @Wenn("Sie auf den Button mit LocatorType {string} und Wert {string} klicken")
    public void wennSieAufDenButtonMitLocatorTypeUndWertKlicken(String locatorType, String value) {
        LocatorType type = LocatorType.valueOf(locatorType);
        BASE_PAGE.clickOnWebElement(BASE_PAGE.DEFAULT_EXPLICIT_WAIT_TIME, value, type, false);
    }

    /**
     * Generic Cucumber step for selecting an item from a dropdown element specified by a locator.
     *
     * @param locatorType the locator strategy to use (must match a value of {@link LocatorType}, e.g.
     *            {@code ID}, {@code CSSSELECTOR})
     * @param value the locator value used to identify the dropdown element
     * @param element the visible text of the dropdown option that should be selected
     */
    @Wenn("Sie im Dropdown mit LocatorType {string} und Wert {string} das Element {string} auswählen")
    public void wennSieImDropdownMitLocatorTypeUndWertDasElementAuswaehlen(String locatorType, String value, String element) {
        LocatorType type = LocatorType.valueOf(locatorType);
        BASE_PAGE.selectDropDownListValueByVisibleText(BASE_PAGE.DEFAULT_EXPLICIT_WAIT_TIME, value, type, element);
    }

    /**
     * Generic Cucumber step for clearing a text field and entering the provided text afterwards.
     *
     * @param locatorType the locator strategy to use (must match a value of {@link LocatorType}, e.g.
     *            {@code NAME}, {@code XPATH})
     * @param value the locator value used to identify the text field
     * @param text the text to enter into the text field
     */
    @Wenn("Sie auf ins Textfeld mit LocatorType {string} und Wert {string} den Text {string} schreiben")
    public void wennSieInsTextfeldMitLocatorTypeUndWertDenTextSchreiben(String locatorType, String value, String text) {
        LocatorType type = LocatorType.valueOf(locatorType);
        BASE_PAGE.enterTextInWebElement(BASE_PAGE.DEFAULT_EXPLICIT_WAIT_TIME, text, value, type, true, false);
    }
}
