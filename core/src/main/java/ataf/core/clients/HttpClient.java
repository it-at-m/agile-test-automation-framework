...
public class HttpClient implements AutoCloseable {
    ...
    /**
     * Creates a concrete {@link HttpClient} (or subclass) for the given target URL by resolving proxy
     * configuration from {@link TestProperties} and {@link DefaultValues}.
     *
     * <p>
     * Resolution order:
     * </p>
     * <ol>
     * <li><b>PAC</b>: If {@code usePac=true}, {@code pacUrl} is used to evaluate a proxy for the
     * given {@code targetUrl}. If {@code pacUrl} is blank or PAC evaluation yields {@code DIRECT},
     ...
     * </p>
     * <ul>
     * <li>If proxy authentication is required, credentials must be provided via
     * {@link AuthenticationHelper}
     * before using the returned client.</li>
     * <li>This method does not validate reachability of the proxy; it only resolves configuration.</li>
     * </ul>
     *
     * @param targetUrl the target URL used for PAC evaluation when enabled
    ...
    public static <T extends HttpClient> T createForTarget(String targetUrl, ClientFactory<T> factory) {
        if (targetUrl == null || targetUrl.isBlank()) {
            throw new IllegalArgumentException("targetUrl must be supplied and must not be blank!");
        }

        boolean usePac = TestProperties.getProperty("usePac", true, String.valueOf(DefaultValues.USE_PAC))
                .map(Boolean::parseBoolean)
                .orElse(DefaultValues.USE_PAC);

        if (usePac) {
            String pacUrl = TestProperties.getProperty("pacUrl", true, DefaultValues.PAC_URL)
                    .map(String::trim)
                    .orElse("");
            try {
                return factory.withPac(pacUrl, targetUrl);
            } catch (UnsupportedOperationException ignored) {
                // If the factory does not support PAC, fall back to direct or proxy resolution below.
            }
        }
        ...
        if (useProxy) {
            ...
            if (!host.isBlank() && port > 0) {
                try {
                    return factory.withProxy(host, port);
                } catch (UnsupportedOperationException ignored) {
                    // Factory doesn't support direct proxy; fall back to direct.
                }
            }
        }
    ...
    /**
     * Convenience factory that creates a plain {@link HttpClient} for the given target URL using
     * proxy/PAC configuration from {@link TestProperties} and {@link DefaultValues}.
     *
     * <p>
     ...
     * {
     *     &#64;code
     *     HttpClient client = HttpClient.createForTarget(targetUrl, new HttpClient.ClientFactory&lt;&gt;() {
     *         &#64;Override
     ...
     *
     *         &#64;Override
     *         public HttpClient withPac(String pacUrl, String targetUrl) {
     *             return new HttpClient(pacUrl, targetUrl);
     *         }
     *
     *         &#64;Override
     *         public HttpClient withProxy(String proxyHostname, int proxyPort) {
     *             return new HttpClient(proxyHostname, proxyPort);
     *         }
     *     });
     * }
     * </pre>
     *
     * @param targetUrl the target URL used for PAC evaluation when PAC is enabled
     * @return a configured {@link HttpClient} instance
     */
    public static HttpClient createForTarget(String targetUrl) {
        return createForTarget(targetUrl, new ClientFactory<>() {
            @Override
            public HttpClient direct() {
                return new HttpClient();
            }
    ...
    private HttpHost getProxyHostByTargetUrl(String pacUrl, String targetUrl) {
        ...
        try {
            ...
            Pattern pattern = Pattern.compile("PROXY ([\\w.-]+):(\\d+)");
        ...
        } catch (ProxyEvaluationException | MalformedURLException | URISyntaxException e) {
    ...
    /**
     * Hook that is called after a response was received and headers are available.
     * <p>
     * Subclasses can override this method to process response headers (e.g. rate limit headers).
     * The default implementation does nothing.
     * </p>
     *
     * @param headers the response headers (never {@code null}, but may be empty)
     */
    protected void onResponseHeaders(Header[] headers) {
        // default: no-op
    }
    ...
    private CloseableHttpResponse executeHttpRequest(HttpUriRequest httpUriRequest) throws IOException {
        ...
        // Hook for subclasses (JiraClient/ConfluenceClient/...)
        onResponseHeaders(lastRequestHeaders);
